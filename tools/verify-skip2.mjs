...
const btn = await page.evaluate(() => {
  ...
  // Find SKIP button by checking all Button containers
  const children = scene.children.list;
  const skip = children.find(c => c.constructor.name === 'Button' && c.list.find?.(o => o.text === 'SKIP'));
  if (!skip) return 'not found';
  return { x: skip.x, y: skip.y, width: skip.width, height: skip.height, scaleX: skip.scaleX, scaleY: skip.scaleY };
});
console.log('SKIP button:', btn);
await browser.close();
